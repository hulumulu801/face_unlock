...
# Сравниваем дискрипторы
    ...
    if 0 <= f_1 <= 0.45: # Если Евклидово расстояние между двумя дексрипторами лиц меньше или равно 0.45, то...
        os.system("loginctl unlock-sessions") # Разблокируем экран блокировки
# Удаляем дискрипторы и фото лица
        shutil.rmtree(abs_path + "Description_dataset", ignore_errors = True)
        shutil.rmtree(abs_path + "dataset", ignore_errors = True)
    elif 0.46 <= f_1 <= 1: # Если Евклидово расстояние между двумя дексрипторами лиц больше 0.46, но меньше 1, удаляем папку и больше ни чего
        shutil.rmtree(abs_path + "Description_dataset", ignore_errors = True)
        shutil.rmtree(abs_path + "dataset", ignore_errors = True)

add_hotkey("Ctrl + A + 4", main) # Вызываем нашу функцию по разбл. с помощью нажатых последовательно "Ctrl + A + 1"
wait("Ctrl + Q") # Выйти из скрипта
